      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <button
            ...
            className="text-[11px] text-gray-500 underline hover:text-gray-600 mt-3"
          >
            ← 로그인 페이지로 돌아가기
          </button>
        </div>
        ...
        <Section title="1. 기본 정보" badge="필수">
          ...
          <Field label="본인에 대해 소개해주세요" hint="성과와 효능감을 증명하지 않아도 되는 공간입니다. 편하게 있는 그대로 💜">
            <textarea value={selfIntro} onChange={e => setSelfIntro(e.target.value)} rows={5}
              className="w-full px-3 py-2.5 bg-gray-50 border border-gray-300 rounded-lg text-gray-800 text-sm focus:outline-none focus:border-amber-500 resize-none" />
          </Field>
        </Section>
            ...
            <div className="space-y-2">
              {REASONS.map(r => (
                <label key={r.key} className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition ${
                  ...
                  <span className="text-xs text-gray-700 leading-relaxed">{r.label}</span>
                </label>
              ))}
        ...
        <Section title="3. 오프 매너 동의" badge="필수">
          <p className="text-xs text-gray-500 mb-3">아래 4가지 오프 매너를 읽고 준수에 동의해주세요.</p>
        ...
        <Section title="4. 운영 정책 및 서비스 이용 안내" badge="필수">
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-4 text-xs leading-relaxed text-gray-600 max-h-64 overflow-y-auto space-y-3">
            <div>
              <p className="text-amber-700 font-semibold mb-1">1. 멤버십 및 환불 정책</p>
            ...
            <div>
              <p className="text-amber-700 font-semibold mb-1">2. 모임 운영 및 인원 기준</p>
            ...
            <div>
              <p className="text-amber-700 font-semibold mb-1">3. 스팟 이용 수칙</p>
              <p>• 현장에서 <strong>1인 1음료 주문 원칙</strong> (멤버 전용 할인 메뉴 제공).</p>
              <p>• 정시 입장 권장. 늦으시면 입장 제한될 수 있습니다.</p>
            </div>
            <div>
              <p className="text-amber-700 font-semibold mb-1">4. 오프 매너 및 안전</p>
        ...
        <Section title="5. 개인정보 수집 및 이용 동의" badge="필수">
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-4 text-xs leading-relaxed text-gray-600 max-h-56 overflow-y-auto space-y-2">
...
function Section({ title, badge, children }: { title: string; badge?: string; children: React.ReactNode }) {
  return (
    <div className="bg-white rounded-xl border border-gray-300 shadow-sm p-5 mb-4">
      <div className="flex items-center gap-2 mb-4">
        <h2 className="text-base font-bold text-amber-800">{title}</h2>
        {badge && <span className="text-[10px] px-2 py-0.5 bg-amber-100 border border-amber-300 text-amber-700 rounded-full">{badge}</span>}
      </div>
...
function Centered({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#FFF8F0] px-4">
      <div className="max-w-md text-center bg-white rounded-2xl border border-gray-300 shadow-sm p-8">
        {children}
      </div>
    </div>
  );
...
export default function OnboardingPage() {
  return (
    <Suspense fallback={<Centered><p className="text-gray-500">로딩 중...</p></Centered>}>
      <OnboardingContent />
    </Suspense>
  );
}
