...
export default function RenewPage() {
  ...
  if (success) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-900/20 via-gray-900 to-gray-900 flex items-center justify-center px-4">
        <div className="text-center">
          <div className="text-5xl mb-4">🎉</div>
          <h1 className="text-xl font-bold text-amber-100 mb-3">{success}</h1>
          <p className="text-gray-400 text-sm mb-6">로그인 후 예약 가능합니다.</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-900/20 via-gray-900 to-gray-900 px-4 py-10">
      <div className="max-w-md mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-amber-100 mb-2">멤버십 갱신</h1>
        ...
        <div className="bg-gray-900/60 backdrop-blur rounded-xl border border-gray-800 p-5 mb-4 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-200 mb-1.5">성함</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-200 mb-1.5">연락처 뒷 4자리</label>
            <input value={phoneLast4} onChange={e => setPhoneLast4(e.target.value.replace(/\D/g, '').slice(0, 4))}
              inputMode="numeric" maxLength={4}
              className="w-full px-3 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:outline-none focus:border-amber-500" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-200 mb-1.5">새 주문번호</label>
